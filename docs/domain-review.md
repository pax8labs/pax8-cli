...
- **Out of scope:** `vendor-provisioning-endpoints.json` (provisioner-facing) and `vendor-usage-endpoints.json` (`/lines`, `/aggregate-lines` — vendor-side usage submission). The CLI consumes partner-side usage via `pax8 usage list`/`show` against `/subscriptions/{id}/usage-summaries` and `/usage-summaries/{id}/usage-lines` (the wire paths line up with the spec as of #337 — prior to that the CLI hit a flat `/usage-summaries` list and a `/lines` leaf, both of which 404'd against the real API; see #212).
