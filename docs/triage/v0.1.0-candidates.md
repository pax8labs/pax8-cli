...
All three commands landed in **PR #443** (`feat(report): reshape reporting surface to Pax8-cost framing per Bret Pittenger review`) on top of the two predecessor PRs (#439 analytics fix, #440 vocab removal). Decisions resolved during implementation:

- **`report renewals`**: shipped as a **new top-level command** (Path a) — clean separation from `subscriptions renewals` (which remains the per-row view).
- **`report subscriptions`** default grouping: **`vendor`** (the dimension `dashboard` doesn't already give). Flags accept `customer | vendor | product | billing-term`.
- **`report concentration`** default grouping: **no default — `--by` required**, accepts `customer | vendor | product`. `--top` defaults to 10; `--threshold` is an alternative filter (when both passed, `--threshold` wins with a stderr `ℹ` notice).

All three commands use wrapped `AmountCurrency` envelopes (`{ amount, currency }`) per the canonical Pax8 wire shape, and carry the standardized partner-revenue disclaimer footer in their `--help`. Demo fixtures in `packages/core/src/mock/demo-data.ts` were extended with a 2-Year and 3-Year sub so the post-#439 normalization fix is visible end-to-end. Inventory companion doc at `docs/triage/reporting-reshape-inventory.md` (the Step 1 deliverable).
...
| B — `recommendations act` heuristic disclosure | < 1 hour (option a) or 1-2 hours (option b) | **Shipped via #425** (option a — help-text disclosure mirror) |
...
| G — Pax8-cost-framed reporting commands | `report subscriptions` 3–4 h; `report renewals` 2–3 h; `report concentration` 2–3 h | **Shipped via #443** (all three) on top of #439 analytics fix + #440 vocab alignment. See "Shipped" subsection above for decisions resolved during implementation |
