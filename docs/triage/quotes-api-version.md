...
| `POST /v2/quotes/{id}/line-items` (addLineItem) | `[{ type: "Standard", productId, quantity, billingTerm? }]` | For Standard: `{ type, productId, quantity, billingTerm, effectiveDate, price }` — **`effectiveDate` and `price` are required and currently missing** | Missing required fields | Open: #312. |
